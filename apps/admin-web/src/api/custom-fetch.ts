...
// 刷新 token（带重试机制、性能监控、友好提示）
async function doRefreshToken(retryCount = 0): Promise<string | null> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      state.setTokens({
        ...
        accessExpiresInSeconds: newTokens.accessExpiresInSeconds,
      });
...
export const customFetch = async <T>(
  ...
  if (response.status === 401 && !isRefreshRequest) {
      ...
      return new Promise((resolve, reject) => {
        subscribeToRefresh((newToken) => {
          if (newToken) {
            ...
            reject(new ApiError({ code: 401, message: "Unauthorized" }, 401));
          }
        });
      });
    }
    ...
    if (newToken) {
      ...
      useAuthStore.getState().logout();
      window.location.href = "/login";
      throw new ApiError({ code: 401, message: "Unauthorized" }, 401);
    }
  }
