...
本项目通过 `main.ts` 中的 `app.setGlobalPrefix('api/v1', { exclude: [...] })` 统一设置全局前缀，Controller 装饰器**只需写资源名**，无需重复添加版本前缀。

- **【强制】业务接口**：Controller 只写资源名（复数形式），系统自动添加 `api/v1` 前缀。
  ```typescript
  // ✅ 正确 - 最终路径为 /api/v1/users
  @Controller('users')
  @Controller('roles')
  @Controller('auth')
  @Controller('orders')

  // ❌ 错误 - 会导致路径重复 /api/v1/api/v1/users
...
- **【强制】嵌套资源**：子资源路径在 Controller 或方法级别定义。
  ...
  class UserController {
    @Get(':userId/roles')  // 最终路径 /api/v1/users/:userId/roles
    getUserRoles() {}
  }
