...
```typescript
// 获取性别字典（简单枚举）
const response = await fetch('/api/v1/dictionaries/type/gender?isEnabled=true');
...
const metaResponse = await fetch('/api/v1/dictionaries/type/app_config/meta?isEnabled=true');
...
if (remoteMeta.configHash !== localHash) {
  // 配置有更新，拉取完整数据
  const fullResponse = await fetch('/api/v1/dictionaries/type/app_config?isEnabled=true');
...
```http
GET /api/v1/dictionaries?page=1&limit=10&type=gender&isEnabled=true
```
...
```http
GET /api/v1/dictionaries/type/:type?isEnabled=true
```
...
**示例**：
```bash
GET /api/v1/dictionaries/type/gender?isEnabled=true
GET /api/v1/dictionaries/type/order_status
```
...
```http
GET /api/v1/dictionaries/type/:type/meta?isEnabled=true
```
...
function useDictionary(type: string) {
  ...
  useEffect(() => {
    fetch(`/api/v1/dictionaries/type/${type}?isEnabled=true`)
      .then(res => res.json())
...
class DictionaryService {
  ...
  async getConfig(type: string, key: string) {
    ...
    // 1. 先查元数据
    const metaRes = await fetch(`/api/v1/dictionaries/type/${type}/meta?isEnabled=true`);
    ...
    // 3. 拉取完整配置
    console.log('🔄 拉取最新配置');
    const fullRes = await fetch(`/api/v1/dictionaries/type/${type}?isEnabled=true`);
...
// 方案 B: 不同 type 对应不同语言
GET /api/v1/dictionaries/type/gender_zh_cn
GET /api/v1/dictionaries/type/gender_en_us
```
...
class DictionaryService {
  ...
  async getConfig(type: string) {
    ...
    if (!this.requestCache.has(key)) {
      const promise = fetch(`/api/v1/dictionaries/type/${type}`)
        .then(res => res.json())
