...
| 模块 | 前缀 | 说明 |
|------|------|------|
| 代理 | `/api/v1/xhs/proxy` | 实时代理小红书 API |
| 直播 | `/api/v1/xhs/live` | 直播数据代理 |
| 同步 | `/api/v1/xhs/sync` | 数据同步管理 |
| 订单 | `/api/v1/xhs/orders` | 本地订单列表与详情 |
| 订单统计 | `/api/v1/xhs/order-stats` | 订单统计与汇总（独立前缀避免路由冲突） |
| 直播统计 | `/api/v1/xhs/stats` | 直播统计查询 |
| 账号 | `/api/v1/xhs/accounts` | 账号管理 |
...
```
GET /api/v1/xhs/live/overview?room_id=xxx&_account=default
```
...
```
GET /api/v1/xhs/live/overview?room_id=6789012345&_account=default
```
...
```
POST /api/v1/xhs/sync/orders/status
```
...
```
GET /api/v1/xhs/orders?page=1&limit=20&startTime=2025-12-01T00:00:00Z&status=已完成
```
...
```
GET /api/v1/xhs/orders/:packageId
```
...
```
GET /api/v1/xhs/order-stats/by-live
```
...
```
GET /api/v1/xhs/order-stats/commission
```
...
```
GET /api/v1/xhs/stats/lives/:roomId
```
...
> **集成说明**：权限控制将集成到现有 RBAC 系统（参考 `docs/features/2025-12-24_rbac/`）。
...
```typescript
// 获取正在直播的房间概览
const overview = await fetch('/api/v1/xhs/live/overview?room_id=6789012345', {
...
```typescript
// 获取本月佣金统计
const stats = await fetch('/api/v1/xhs/order-stats/commission?startTime=2025-12-01T00:00:00Z&groupBy=day', {
  headers: { 'Authorization': `Bearer ${token}` }
}).then(r => r.json());
...
```typescript
// 同步最近30天订单
const result = await fetch('/api/v1/xhs/sync/orders', {
  method: 'POST',
  headers: {
...
- **API 文档**: `docs/features/2025-12-26_xiaohongshu/api.md`（本文档）
- **数据结构**: `docs/features/2025-12-26_xiaohongshu/schema.md`
...
async function syncAndWait(accountTag = 'default') {
  try {
    await fetch('/api/v1/xhs/sync/orders', {
      method: 'POST',
...
```typescript
// 获取账号列表
const accounts = await fetch('/api/v1/xhs/accounts', {
  headers: { 'Authorization': `Bearer ${token}` }
}).then(r => r.json());

// 渲染账号选择器
accounts.data.forEach(acc => {
  console.log(`${acc.name} (${acc.tag}) - ${acc.isExpired ? '已过期' : `剩余 ${acc.daysUntilExpiry} 天`}`);
});
```
