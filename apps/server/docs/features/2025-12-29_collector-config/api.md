...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/v1/collector/rules` | 规则列表（分页） |
| GET | `/api/v1/collector/rules/:id` | 规则详情 |
| POST | `/api/v1/collector/rules` | 创建规则 |
| PUT | `/api/v1/collector/rules/:id` | 更新规则 |
| DELETE | `/api/v1/collector/rules/:id` | 删除规则 |
| PATCH | `/api/v1/collector/rules/:id/toggle` | 切换启用状态 |
| PATCH | `/api/v1/collector/rules/batch-toggle` | 批量切换启用状态 |
| POST | `/api/v1/collector/rules/test-match` | 域名匹配测试 |

### 配置下发 API（需 X-Script-Token 认证）

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/v1/collector/config` | 获取所有启用的规则 |
...
```
DELETE /api/v1/collector/rules/:id
```
...
```
PATCH /api/v1/collector/rules/:id/toggle
```
...
```
PATCH /api/v1/collector/rules/batch-toggle
```
...
```
POST /api/v1/collector/rules/test-match
```
...
async function getCollectorConfig() {
  ...
  return new Promise((resolve, reject) => {
    GM_xmlhttpRequest({
      method: 'GET',
      url: 'https://i.54kb.com/api/v1/collector/config',
      headers: {
      ...
      onload: function(response) {
        ...
        if (response.status === 200) {
          ...
          const etag = response.responseHeaders.match(/etag:\s*"([^"]+)"/i)?.[1];
