...
**接口设计**：`GET /v1/config/public/:namespace/:key`
...
```typescript
// 注意：全局前缀 'v1' 已在 main.ts 中配置，Controller 路径不需要重复
@ApiTags('Script Data')
@Controller('script')
@Public()
...
```bash
GET /v1/config/public/app-settings/home_modules
# 无需认证
```
...
# 下载
GET /v1/script/download/xhs_cookies_account_a
X-Script-Token: your-32-char-token

# 列表
GET /v1/script/list?page=1&limit=10
X-Script-Token: your-32-char-token
```
