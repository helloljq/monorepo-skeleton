...
| 方法   | 路由                          | 功能                   | 权限                      |
| ------ | ----------------------------- | ---------------------- | ------------------------- |
| GET    | `/v1/config/namespaces`       | 查询命名空间列表       | `config:read`             |
| GET    | `/v1/config/namespaces/:name` | 获取命名空间详情       | `config:read`             |
| POST   | `/v1/config/namespaces`       | 创建命名空间           | `config:namespace:create` |
| PATCH  | `/v1/config/namespaces/:name` | 更新命名空间           | `config:namespace:update` |
| DELETE | `/v1/config/namespaces/:name` | 删除命名空间（软删除） | `config:namespace:delete` |
...
| 方法   | 路由                              | 功能                   | 权限            |
| ------ | --------------------------------- | ---------------------- | --------------- |
| GET    | `/v1/config/:namespace`           | 获取命名空间下所有配置 | `config:read`   |
| GET    | `/v1/config/:namespace/:key`      | 获取单个配置项         | `config:read`   |
| GET    | `/v1/config/:namespace/:key/meta` | 获取配置元数据（轻量） | `config:read`   |
| POST   | `/v1/config/:namespace`           | 创建配置项             | `config:write`  |
| PUT    | `/v1/config/:namespace/:key`      | 更新配置项             | `config:write`  |
| DELETE | `/v1/config/:namespace/:key`      | 删除配置项             | `config:delete` |

### 2.3 版本与历史

| 方法 | 路由                                           | 功能             | 权限              |
| ---- | ---------------------------------------------- | ---------------- | ----------------- |
| GET  | `/v1/config/:namespace/:key/history`           | 获取配置变更历史 | `config:read`     |
| POST | `/v1/config/:namespace/:key/rollback/:version` | 回滚到指定版本   | `config:rollback` |

### 2.4 批量操作

| 方法 | 路由                                     | 功能              | 权限           |
| ---- | ---------------------------------------- | ----------------- | -------------- |
| GET  | `/v1/config/:namespace/batch?keys=a,b,c` | 批量获取多个配置  | `config:read`  |
| POST | `/v1/config/:namespace/batch`            | 批量创建/更新配置 | `config:write` |

> **限制**：批量获取 `keys` 参数最多 50 个，超出返回 `BAD_REQUEST`。

### 2.5 Meta 接口返回结构

`GET /v1/config/:namespace/:key/meta` 返回轻量级元数据：
...
async function getConfigWithCache(namespace: string, key: string) {
  ...
  // 1. 获取 meta（~100B）
  const meta = await fetch(`/v1/config/${namespace}/${key}/meta`);
  ...
  const config = await fetch(`/v1/config/${namespace}/${key}`);
...
export class ConfigCenterGateway {
  ...
  async handleSubscribe(
    ...
    if (!hasPermission) {
      return {
        success: false,
        error: "Forbidden: missing config:read permission",
      };
    }
...
// 无权限用户读取
GET /v1/config/third_party/xhs_cookies
{
  "code": 0,
  "data": {
    "key": "xhs_cookies",
...
```typescript
// 查看历史
GET /v1/config/mini_program/home_banners/history
{
  "code": 0,
  "data": [
...
// 回滚到版本 2
POST /v1/config/mini_program/home_banners/rollback/2
{
