...
```typescript
const API_BASE_URL = "http://localhost:8100/v1";
...
async function getNamespaces(params?: {
  ...
  const response = await fetch(`${API_BASE_URL}/config/namespaces?${query}`, {
...
async function getConfig(namespace: string, key: string) {
  const response = await fetch(`${API_BASE_URL}/config/${namespace}/${key}`, {
    headers: { Authorization: `Bearer ${ACCESS_TOKEN}` },
  });
...
- [配置中心功能设计](./task.md)
- [配置中心实现方案](./implementation-plan.md)
...
```typescript
const NAMESPACE =
  process.env.NODE_ENV === "production" ? "app_prod" : "app_dev";
