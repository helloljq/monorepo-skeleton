...
export class PermissionsGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // 获取用户权限 (基于角色缓存)
    const userPermissions = await this.permissionCache.getUserPermissions(user.roles);
...
```
// 构造授权 URL
GET /api/v1/auth/wechat/h5/authorize?redirectUri=xxx
Response: { authorizeUrl }

// 处理授权回调
POST /api/v1/auth/wechat/h5/callback
Body: { code, deviceId }
```
