...
| 接口                             | 方法   | 认证 | 说明             |
| -------------------------------- | ------ | ---- | ---------------- |
| `/v1/collector/rules`            | GET    | JWT  | 规则列表（分页） |
| `/v1/collector/rules/:id`        | GET    | JWT  | 规则详情         |
| `/v1/collector/rules`            | POST   | JWT  | 创建规则         |
| `/v1/collector/rules/:id`        | PUT    | JWT  | 更新规则         |
| `/v1/collector/rules/:id`        | DELETE | JWT  | 删除规则         |
| `/v1/collector/rules/:id/toggle` | PATCH  | JWT  | 启用/禁用规则    |

### 配置下发接口

| 接口                   | 方法 | 认证           | 说明                                 |
| ---------------------- | ---- | -------------- | ------------------------------------ |
| `/v1/collector/config` | GET  | X-Script-Token | 获取所有启用的规则（供油猴脚本调用） |
